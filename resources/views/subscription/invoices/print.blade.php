...
<div class="invoice-preview">
    <div class="invoice-preview-container bg-white mt15">
        ...
        <table style="color: #444; width: 100%;">
            ...
            <tr class="invoice-preview-header-row">
                <td style="width: 45%; vertical-align: top;">
                    @if(!empty($logo=\Illuminate\Support\Facades\DB::connection('mysql')->table('settings')->where('setting_key','company_logo')->first()->setting_value))
                ...
                <td class="invoice-info-container invoice-header-style-one"
                    ...
                    @if($invoice->status==='partially_paid')
                        ...
                        -
                        <a href="{{config('app.app_protocol')}}{{tenant()->domains->first()->domain}}.{{config('app.central_domain')}}/setting/subscription/invoice/{{$invoice->id}}/show"><span
            ...
            <tr>
                <td>
                    <div>
                        <b>{{\Illuminate\Support\Facades\DB::connection('mysql')->table('settings')->where('setting_key','company_name')->first()->setting_value}}</b>
                    </div>
                    <div style="line-height: 3px;"></div>
                    <span class="invoice-meta" style="font-size: 90%; color: #666;">
                                        {{\Illuminate\Support\Facades\DB::connection('mysql')->table('settings')->where('setting_key','company_address')->first()->setting_value}}
                                        <br>
                                        Phone: {{\Illuminate\Support\Facades\DB::connection('mysql')->table('settings')->where('setting_key','company_tel')->first()->setting_value}}
                                        <br>
                                        Website: <a
                            style="color:#666; text-decoration: none;"
                            href="{{\Illuminate\Support\Facades\DB::connection('mysql')->table('settings')->where('setting_key','company_website')->first()->setting_value}}">{{\App\Models\Setting::where('setting_key','company_website')->first()->setting_value}}</a>
                                    </span>
                </td>
                <td></td>
                <td>
        ...
        <table class="table-responsive" style="width: 100%; color: #444;">
            ...
            <tr>
                <td colspan="4"
                    style="text-align: right;">Sub Total
                </td>
            ...
            @if($invoice->discount_amount>0)
                <tr>
                    <td colspan="4"
                        style="text-align: right;">Discount
                    </td>
                    <td style="text-align: right; width: 20%; border: 1px solid #fff; background-color: #f4f4f4;">
            ...
            <tr>
                ...
                <td style="text-align: right; width: 20%; border: 1px solid #fff; background-color: #f4f4f4;">
                    {{number_format($invoice->amount-$invoice->balance,2)}}
                </td>
            </tr>
            <tr>
                <td colspan="4"
                    style="text-align: right;">Balance
                </td>
        ...
        <table class="table-responsive" style="width: 100%; color: #444;">
            <tbody>
            <tr style="font-weight: bold; background-color: #2AA384; color: #fff;  ">
                <th style="text-align: center;   border-right: 1px solid #eee;"> #</th>
            ...
            @foreach($invoice->invoice_payments as $key)
                <tr style="background-color: #f4f4f4; ">
                    <td style="text-align: center;   border: 1px solid #fff; padding: 10px;">
                        {{$key->id}}
                    </td>
