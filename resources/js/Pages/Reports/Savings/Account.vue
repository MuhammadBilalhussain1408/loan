        ...
        <div class=" mx-auto">
            <div class="bg-white rounded shadow">
                <div class="bg-gray-50 px-6 py-2 flex border-b">
                    <a :href="route('reports.savings.account',form)+'&download=1&download_type=pdf'"
                       class="text-red-400">
                        <font-awesome-icon icon="file-pdf"></font-awesome-icon>
                    </a>
                    <a :href="route('reports.savings.account',form)+'&download=1&download_type=excel'"
                       class="ml-2 text-green-400">
                        <font-awesome-icon icon="file-excel"></font-awesome-icon>
                    </a>
                    <a :href="route('reports.savings.account',form)+'&download=1&download_type=csv'"
                       class="ml-2 text-green-400">
                    ...
                    <table class="w-full whitespace-no-wrap">
                        ...
                        <tr v-for="result in results.data" :key="result.id"
                            ...
                            <td class="border-t p-2">
                                <inertia-link
                                              :href="route('clients.show', result.client_id)"
                                              tabindex="-1" class="text-indigo-600 hover:text-indigo-900">
                                    {{ result.client }}
                                </inertia-link>
                            </td>
                            <td class="border-t p-2">
                                <inertia-link v-if=" result.savings_officer_id"
                                              :href="route('users.show', result.savings_officer_id)"
                                              tabindex="-1" class="text-indigo-600 hover:text-indigo-900">
                                    {{ result.savings_officer }}
                                </inertia-link>
                            </td>
                            <td class="border-t p-2">
                                <inertia-link
                                    :href="route('savings.show', result.id)"
                                    tabindex="-1" class="text-indigo-600 hover:text-indigo-900">
                                    {{ result.id }}
                                </inertia-link>
                            </td>
                            ...
                            <td class="border-t p-2">
                             <span>
                                {{ result.submitted_on_date }}
                            </span>
                            </td>
                        </tr>
                        <tr v-if="results.data.length === 0">
                            <td class="border-t px-6 py-4 text-center" colspan="8">No results found.</td>
                        </tr>
                        </tbody>
                    </table>
                </div>
...
export default {
    ...
    methods: {
        reset() {
            ...
            this.$inertia.get(this.route('reports.savings.account', Object.keys(query).length ? query : {}))
        },
        apply() {
            let query = pickBy(this.form)
            this.$inertia.get(this.route('reports.savings.account', Object.keys(query).length ? query : {}))
