    ...
    <app-layout>
        ...
        <div class="mx-auto bg-white p-2 mb-4">
            <div class="grid grid-cols-1 md:grid-cols-5 gap-2">
                ...
                <div>
                    <jet-label for="savings_id" value="Savings Account"/>
                    <Multiselect
                        :required="true"
                        v-model="form.savings_id"
                        :options="savings"
        ...
        <div class=" mx-auto" v-if="results.data">
            <div class="bg-white rounded shadow">
                <div class="bg-gray-50 px-6 py-2 flex border-b">
                    <a :href="route('reports.savings.account_statement',form)+'&download=1&download_type=pdf'"
                       class="text-red-400">
                        <font-awesome-icon icon="file-pdf"></font-awesome-icon>
                    </a>
                    <a :href="route('reports.savings.account_statement',form)+'&download=1&download_type=excel'"
                       class="ml-2 text-green-400">
                        <font-awesome-icon icon="file-excel"></font-awesome-icon>
                    </a>
                    <a :href="route('reports.savings.account_statement',form)+'&download=1&download_type=csv'"
                       class="ml-2 text-green-400">
...
const fetchClients = async (query) => {
    let where = ''
    const response = await fetch(
        route('clients.search') + '?with_savings=1&s=' + query,
        {}
    );
...
export default {
    ...
    data() {
        return {
            form: {
                ...
                savings_id: this.filters.savings_id,
    ...
    methods: {
        reset() {
            ...
            this.$inertia.get(this.route('reports.savings.account_statement', Object.keys(query).length ? query : {}))
        },
        apply() {
            let query = pickBy(this.form)
            this.$inertia.get(this.route('reports.savings.account_statement', Object.keys(query).length ? query : {}))
        ...
        changeClient() {
            ...
            this.savings = []
            this.savings = this.selectedClient.savings
        },
    },
    watch: {}
}
