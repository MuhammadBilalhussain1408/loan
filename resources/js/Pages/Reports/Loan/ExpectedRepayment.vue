<template>
    <app-layout>
        ...
        <div class=" mx-auto">
            <div class="bg-white rounded shadow">
                <div class="bg-gray-50 px-6 py-2 flex border-b">
                    <a :href="route('reports.loans.expected_repayment',form)+'&download=1&download_type=pdf'"
                       class="text-red-400">
                        <font-awesome-icon icon="file-pdf"></font-awesome-icon>
                    </a>
                    <a :href="route('reports.loans.expected_repayment',form)+'&download=1&download_type=excel'"
                       class="ml-2 text-green-400">
                        <font-awesome-icon icon="file-excel"></font-awesome-icon>
                    </a>
                    <a :href="route('reports.loans.expected_repayment',form)+'&download=1&download_type=csv'"
                       class="ml-2 text-green-400">
                ...
                <div class="overflow-auto">
                    <table class="w-full whitespace-no-wrap">
                        ...
                        <tr v-for="result in results.data" :key="result.id"
                            ...
                            <td class="border p-2">
                             <span>
                                {{ $filters.currency(result.interest) }}
                            </span>
                            </td>
                            <td class="border p-2">
                             <span>
                                {{ $filters.currency(result.fees) }}
                            </span>
                            </td>
                            <td class="border p-2">
                             <span>
                                {{ $filters.currency(result.penalties) }}
                            </span>
                            </td>
                            <td class="border p-2">
                             <span>
                            ...
                            <td class="border p-2">
                             <span>
                                {{ $filters.currency(result.principal_repaid_derived) }}
                            </span>
                            </td>
                            <td class="border p-2">
                             <span>
                                {{ $filters.currency(result.interest_repaid_derived) }}
                            </span>
                            </td>
                            <td class="border p-2">
                             <span>
                                {{ $filters.currency(result.fees_repaid_derived) }}
                            </span>
                            </td>
                            <td class="border p-2">
                             <span>
                                {{ $filters.currency(result.penalties_repaid_derived) }}
                            </span>
                            </td>
                            <td class="border p-2">
                             <span>
                                {{ $filters.currency(result.total-result.balance) }}
                            </span>
                            </td>
                            <td class="border p-2">
                             <span>
                        ...
                        <tr v-if="results.data.length === 0">
                            <td class="border-t px-6 py-4 text-center" colspan="12">No results found.</td>
                        </tr>
                        </tbody>
                    </table>
                </div>
        ...
        <teleport to="head">
            <title>Expected Repayments</title>
        </teleport>
    </app-layout>
</template>
...
export default {
    ...
    methods: {
        reset() {
            ...
            this.$inertia.get(this.route('reports.loans.expected_repayment', Object.keys(query).length ? query : {}))
        },
        apply() {
            let query = pickBy(this.form)
            this.$inertia.get(this.route('reports.loans.expected_repayment', Object.keys(query).length ? query : {}))
