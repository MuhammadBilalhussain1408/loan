    ...
    <app-layout>
        <template #header>
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">
                ...
                <inertia-link class="text-indigo-400 hover:text-indigo-600" :href="route('reports.loans.index')">Loan Reports
                </inertia-link>
                <span class="text-indigo-400 font-medium">/</span>
                Collection Sheet
            </h2>
        ...
        <div class=" mx-auto">
            <div class="bg-white rounded shadow">
                <div class="bg-gray-50 px-6 py-2 flex border-b">
                    <a :href="route('reports.loans.collection_sheet',form)+'&download=1&download_type=pdf'"
                       class="text-red-400">
                        <font-awesome-icon icon="file-pdf"></font-awesome-icon>
                    </a>
                    <a :href="route('reports.loans.collection_sheet',form)+'&download=1&download_type=excel'"
                       class="ml-2 text-green-400">
                        <font-awesome-icon icon="file-excel"></font-awesome-icon>
                    </a>
                    <a :href="route('reports.loans.collection_sheet',form)+'&download=1&download_type=csv'"
                       class="ml-2 text-green-400">
                    ...
                    <table class="w-full whitespace-no-wrap">
                        ...
                        <tr v-for="result in results.data" :key="result.id"
                            ...
                            <td class="border-t p-2">
                                <inertia-link v-if="result.loan && result.loan.loan_officer"
                                              :href="route('users.show', result.loan.loan_officer.id)"
                                              tabindex="-1" class="text-indigo-600 hover:text-indigo-900">
                                    {{ result.loan.loan_officer.name }}
                                </inertia-link>
                            </td>
                            <td class="border-t p-2">
                              <span v-if="result.loan && result.loan.branch">
                                {{ result.loan.branch.name }}
                            </span>
                            </td>
                            <td class="border-t p-2">
                            ...
                            <td class="border-t p-2">
                             <span v-if="result.loan && result.loan.client">
                                {{ result.loan.client.mobile }}
                            </span>
                            </td>
                            <td class="border-t p-2">
                            ...
                            <td class="border-t p-2">
                             <span v-if="result.loan && result.loan.product">
                                {{ result.loan.product.name }}
                            </span>
                            </td>

                            <td class="border-t p-2">
                             <span v-if="result.loan">
                                {{ result.loan.expected_maturity_date }}
                            </span>
                            </td>
                            <td class="border-t p-2">
                             <span>
                            ...
                            <td class="border-t p-2">
                             <span>
                                {{ $filters.currency(result.total) }}
                            </span>
                            </td>
                            <td class="border-t p-2">
                             <span>
                                {{ $filters.currency(result.total_due) }}
                            </span>
                            </td>
                        </tr>
                        <tr v-if="results.data.length === 0">
                            <td class="border-t px-6 py-4 text-center" colspan="10">No results found.</td>
                        </tr>
                        </tbody>
                    </table>
                </div>
...
const fetchPractitioners = async (query) => {
    let where = ''
    const response = await fetch(
        route('referring_practitioners.search') + '?s=' + query,
        {}
    );

    const data = await response.json(); // Here you have the data that you need
    return data.map((item) => {
        return {value: item.id, label: item.name + (item.practice_number ? '(' + item.practice_number + ')' : '')}
    })
}
...
export default {
    ...
    methods: {
        reset() {
            ...
            this.$inertia.get(this.route('reports.loans.collection_sheet', Object.keys(query).length ? query : {}))
        },
        apply() {
            let query = pickBy(this.form)
            this.$inertia.get(this.route('reports.loans.collection_sheet', Object.keys(query).length ? query : {}))
