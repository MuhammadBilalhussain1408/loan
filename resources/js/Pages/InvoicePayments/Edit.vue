    ...
    <app-layout>
        <template #header>
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">
                ...
                <inertia-link class="text-indigo-400 hover:text-indigo-600" :href="route('billing.invoices.show',invoicePayment.invoice_id)">
                    Invoice
                    #{{ invoicePayment.invoice_id }}
                </inertia-link>
                <span class="text-indigo-400 font-medium">/</span> Edit Invoice Payment
            </h2>
        </template>
        <div class="">
            <div class=" mx-auto">
    ...
    data() {
        return {
            form: this.$inertia.form({
                paid_by: this.invoicePayment.paid_by,
                co_payer_id: this.invoicePayment.co_payer_id,
                trans_id: this.invoicePayment.trans_id,
                amount: this.invoicePayment.amount,
                date: this.invoicePayment.date,
                payment_type_id: this.invoicePayment.payment_detail.payment_type_id,
                reference: this.invoicePayment.payment_detail.reference,
                account_number: this.invoicePayment.payment_detail.account_number,
                routing_code: this.invoicePayment.payment_detail.routing_code,
                receipt: this.invoicePayment.payment_detail.receipt,
                bank_name: this.invoicePayment.payment_detail.bank_name,
                description: this.invoicePayment.description,
                print_payment: false,
            }),
            pageTitle: "Edit Payment",
            pageDescription: "Edit Payment",
        }

    },
    methods: {
        submit() {
            this.form.put(this.route('billing.payments.update', this.invoicePayment.id), {})
