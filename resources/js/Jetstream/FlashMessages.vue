<template>
    <div>
        <div v-if="$page.props.flash.success && show" class="mb-8 flex items-center justify-between bg-green-500 rounded w-full">
        ...
        <div v-if="($page.props.flash.error || Object.keys($page.props.errors).length > 0) && show" class="mb-8 flex items-center justify-between bg-red-500 rounded w-full">
            <div class="flex items-center">
                ...
                <div v-else class="py-4 text-white text-sm font-medium">
                    ...
                    <ul v-for="error in Object.keys($page.props.errors)">
                        <li>{{$page.props.errors[error]}}</li>
                    </ul>
                </div>
            </div>
            <button type="button" class="group mr-2 p-2" @click="show = false">
