...
class LoanScheduleController extends Controller
{
    ...
    public function index(Loan $loan)
    {
        ...
        foreach ($loan->schedules as $ind => $item) {
            ...
            $item->days = $loan->repayment_frequency_type == 'ballon_payment' ? 4*365 : Carbon::parse($item->due_date)->diffInDays(Carbon::parse($item->from_date));
        }
        ...
        $loan->arrears_days = $loan->repayment_frequency_type == 'ballon_payment' ? 4*365 :$arrearsDays;
